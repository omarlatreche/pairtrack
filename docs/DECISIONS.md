...
**Why:** a pack this size is well under 1MB, so there is no performance argument for
per-record encryption. One blob means one IV per write instead of one per job, a much smaller
surface for the one mistake that breaks AES-GCM. It also hides the record count, which
per-record encryption would leak.

**Cost:** every save rewrites the whole blob. Measured at well under 20ms for a pack of a few hundred,
...
`undefined`. That column becomes empty on every row with nothing reported,
...
an upstream source is routine. On `Circuit` it would silently erase every
...
vault** — every job number, every circuit number — alive on the
...
important identifier, not job number. Column A (`JOB`) is a row index and
