...
const ALLOWED = [
  ...
  /^https?:\/\/relaxng\.org\//,
  // OOXML / ODF namespace roots. These appear as XML namespace IDENTIFIERS
  // inside SheetJS — they name a schema, they are never dereferenced. The
  // network-API check below is what actually guarantees nothing is fetched.
  /^https?:\/\/purl\.oclc\.org\//,
  /^https?:\/\/[a-z.]*openxmlformats\.org\//,
...
/**
 * Network APIs that must not appear in the bundle at all.
 *
 * This is the stronger half of the check. An allowlisted origin string is a
 * judgement call about intent; a bundle with no way to open a socket is a fact.
 * BRIEF §3.2 is "no network requests at runtime" — so verify the capability is
 * absent, not merely that the URLs look innocent.
 */
const NETWORK_APIS = [
...
/**
 * sw.js is exempt: a service worker's whole job is to intercept requests, and
 * it must be able to fetch a new app version. It is generated by
 * scripts/build-sw.mjs and is short enough to read in full.
 */
const NETWORK_EXEMPT = [/(^|\/)sw\.js$/];
...
for (const file of files) {
  ...
  if (!NETWORK_EXEMPT.some((re) => re.test(file))) {
    for (const { name, re } of NETWORK_APIS) {
      if (re.test(text)) {
        violations.push({ file, url: name, why: 'network API present in bundle output' });
      }
    }
  }
...
if (violations.length > 0) {
  ...
  console.error('  The bundle can reach the network, or names an origin outside the app.');
  console.error('  This breaks the aeroplane-mode guarantee (BRIEF §3.2) and the CSP (§9.7).\n');
