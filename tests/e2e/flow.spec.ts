...
test('setup, import, mark, lock, unlock, persistence, export', async ({ page }) => {
  ...
  // --- One tap marks a job -------------------------------------------------
  const firstJobNumber = (await page.locator('.card__number').first().textContent())?.trim() ?? '';
  expect(firstJobNumber).not.toBe('');

  await page.locator('button.mark--pass').first().click();

  await expect(page.locator('.toast')).toBeVisible();
  await expect(page.getByRole('button', { name: /^Activated/ })).toContainText('1');
  await expect(page.getByRole('button', { name: /^Outstanding/ })).toContainText(
    String(TOTAL_JOBS - 1),
  );

  // The timestamp is written by the app, not typed.
  await expect(page.locator('.card').first().locator('.status--activated')).toBeVisible();

  // --- Undo restores it ----------------------------------------------------
  await page.getByRole('button', { name: 'Undo' }).click();
  await expect(page.getByRole('button', { name: /^Outstanding/ })).toContainText(String(TOTAL_JOBS));

  // --- Mark 20 jobs, as the brief's offline check requires ------------------
  //
  // Filtered to Outstanding first, which is how he would actually work: each
  // job he ticks leaves the filter and the next one moves up under his thumb.
  // Tapping the same card repeatedly would instead walk ONE job through its
  // three gates, which is also correct behaviour but is not this check.
  ...
  await expect(page.getByRole('button', { name: /^Activated/ })).toContainText('20');

  // --- Search --------------------------------------------------------------
  const search = page.getByLabel('Search jobs');
  await search.fill(firstJobNumber.replace('/', ' ').toLowerCase());
  ...
  const isDescending = async (locator: typeof page) => {
    const numbers = (await locator.locator('.card__number').allTextContents()).slice(0, 8);
  ...
  expect(bodyAfterLock).not.toContain(firstJobNumber.toUpperCase());
...
test('IndexedDB holds no readable job data', async ({ page }) => {
  ...
  // Put a distinctive note on a job, then confirm it is not readable on disk.
  await page.locator('.card__body').first().click();
  await expect(page.getByRole('heading', { name: firstJobNumber })).toBeVisible();
  await page.getByLabel('Notes').fill('CANARY-NOTE-MUST-NOT-BE-READABLE');
  ...
  for (const needle of [
    'CANARY-NOTE-MUST-NOT-BE-READABLE',
    firstJobNumber,
    'QQA',
    '02079460',
    ENGINEER,
...
test('a malformed row is flagged and correctable in the app', async ({ page }) => {
  ...
  // Open the row whose bar pair is a bare 0.
